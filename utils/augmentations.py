...
class ToPercentCoords(object):
    def __call__(self, image, masks=None, boxes=None, labels=None):
        height, width, channels = image.shape
        boxes[:, 0] = np.divide(boxes[:, 0], width)
        boxes[:, 2] = np.divide(boxes[:, 2], width)
        boxes[:, 1] = np.divide(boxes[:, 1], height)
        boxes[:, 3] = np.divide(boxes[:, 3], height)
...
class Resize(object):
    ...
    def __call__(self, image, masks, boxes, labels=None):
        ...
        if self.resize_gt:
            ...
            boxes[:, [0, 2]] = np.multiply(boxes[:, [0, 2]], (width  / img_w))
            boxes[:, [1, 3]] = np.multiply(boxes[:, [1, 3]], (height / img_h))
